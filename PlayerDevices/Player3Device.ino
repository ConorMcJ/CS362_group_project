...
// Dedicated line to SIMON (matches Simon Option-2)
const int rxPin = 6;   // from Simon TX=7
const int txPin = 7;   // to Simon RX=6
...
// 4 LED-game buttons (R,G,Y,B order)
...
void startCountdown(int sec) {
  timeLeftSec = sec;
  responseEndTime = millis() + (unsigned long)sec * 1000UL;
...
void handleS_REQ(const String& rest) {
  ...
  if (type == "LED") {
    mySerial.println("P_RESP,3,LED," + ledAnswer);
    pState = P_WAIT_RESULT;
  }
  else if (type == "BUZ") {
    mySerial.println("P_RESP,3,BUZ," + buzAnswer);
    pState = P_WAIT_RESULT;
  }
  else if (type == "US") {
    long duration;
    ...
    mySerial.println("P_RESP,3,US," + String(dist));
    pState = P_WAIT_RESULT;
  }
  else if (type == "MEM") {
    mySerial.println("P_RESP,3,MEM," + recallAnswer);
    pState = P_WAIT_RESULT;
  }
}
...
void handleBuzzerInput() {
  ...
  if (now - lastJoyInputTime > joyInputDelay) {
    if      (y < JOY_LOW_T)  { buzAnswer += 'H'; lastJoyInputTime = now; }
    else if (y > JOY_HIGH_T) { buzAnswer += 'L'; lastJoyInputTime = now; }
  }
}
